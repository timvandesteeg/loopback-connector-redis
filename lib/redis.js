...
exports.initialize = function initializeSchema(dataSource, callback) {
	...
	clientWrapper.ttl = dataSource.settings.options.ttl || -1;
	clientWrapper.database = database;
...
BridgeToRedis.prototype.save = function(model, data, callback) {
    ...
    this.client.hgetall(model + ':' + data.id, function(err, prevData) {
		...
		var key = model + ':' + data.id;
        this.client.hmset([key, data], function(err) {
            if (err) return callback(err);
			
			//if options.ttl is set for datasource, add expire ttl for each save()
			if (this.client.ttl > -1) {
				this.client.expire([key, this.client.ttl], function(err) {
					if (err) return callback(err);
				});
			}
...
BridgeToRedis.prototype.count = function count(model, callback, where) {
    if (where && Object.keys(where).length) {
		...
		var regex = 'db' + this.client.database + '.*expires=([0-9]*)'; //e.g. db9:keys=6,expires=1,avg_ttl=100
		this.client.info('keyspace', function(err, value) {
			var match = value.match(regex);
			var count = (match.length > 1 ? match[1] : 0);
			callback(err, count);
		});
...
function deleteNulls(data) {
	if (data instanceof Array)
		return data.filter(n => n); //remove empty, null and undefined
